...
GENERATE_TIMEOUT_S = int(os.environ.get("SCENEMA_GENERATE_TIMEOUT_S", "7200"))
...
def _format_status(progress: dict, elapsed_s: float) -> str:
...
def _post_api(payload: dict) -> dict:
...
def _decode_result(result: dict) -> tuple:
    """Decode /generate result into Gradio audio and metadata."""
    ...
    meta_display = {
        ...
        "saved_wav": meta.get("saved_wav", ""),
        "saved_metadata": meta.get("saved_metadata", ""),
    }

    return (sample_rate, audio_data), meta_display


def _poll_progress(job_id: str) -> dict:
    req = urllib.request.Request(f"{API_URL}/progress/{job_id}")
...
def cancel_job(job_id: str):
    ...
    req = urllib.request.Request(f"{API_URL}/cancel/{job_id}", method="POST")
    try:
        with urllib.request.urlopen(req, timeout=5) as resp:
            result = json.loads(resp.read())
        return f"**Cancelling**\n\nStop requested for `{result.get('job_id', job_id)}`."
    except Exception as e:
        return f"Cancel request failed: {e}"


def _call_api_with_progress(payload: dict, prompt: str | None, include_prompt: bool):
    """Run /generate in a background thread and yield UI progress updates."""
    ...
    def worker():
        try:
            state["result"] = _post_api(payload)
        except Exception as e:
            state["error"] = e
    ...
    last_progress = {
        "percent": 0,
        "stage": "Queued",
        "detail": "Waiting for server worker",
    }

    while thread.is_alive():
        try:
            last_progress = _poll_progress(job_id)
        except Exception:
            pass
        status = _format_status(last_progress, time.time() - started)
        if include_prompt:
            yield None, None, prompt, status, job_id
        else:
            yield None, None, status, job_id
        time.sleep(1.0)
    ...
    audio, meta = _decode_result(state["result"])
    status = _format_status(
        {
            "percent": 100,
            "stage": "Complete",
            "detail": "Audio is ready",
        },
        time.time() - started,
    )
    if include_prompt:
        yield audio, meta, prompt, status, ""
    else:
        yield audio, meta, status, ""
...
def generate(
    ...
    yield from _call_api_with_progress(payload, prompt, include_prompt=True)
...
def voice_clone(
    ...
    if not ref_path or not os.path.isfile(ref_path):
        raise gr.Error("Reference audio expired or is missing. Re-upload the file.")
...
def generate_raw(
    ...
    yield from _call_api_with_progress(payload, None, include_prompt=False)
        ...
        with gr.Tabs():
            ...
            with gr.Tab("Generate"):
                ...
                gen_event = gen_btn.click(
                    ...
                    outputs=[gen_audio, gen_meta, gen_xml, gen_status, active_job],
                )
                gen_stop.click(
                    fn=cancel_job,
                    inputs=[active_job],
            ...
            with gr.Tab("Voice Design"):
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        with gr.Row():
                            ...
                            vd_stop = gr.Button("Stop")

                    with gr.Column(scale=1):
                        vd_status = gr.Markdown("Idle")
                ...
                vd_event = vd_btn.click(
                    fn=voice_design,
                    inputs=[
                        vd_voice, vd_text, vd_scene,
                        vd_language, vd_seed,
                    ],
                    outputs=[vd_audio, vd_meta, vd_xml, vd_status, active_job],
                )
                vd_stop.click(
                    fn=cancel_job,
                    inputs=[active_job],
            ...
            with gr.Tab("Voice Cloning"):
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        with gr.Row():
                            ...
                            vc_stop = gr.Button("Stop")

                    with gr.Column(scale=1):
                        vc_status = gr.Markdown("Idle")
                ...
                vc_event = vc_btn.click(
                    ...
                    outputs=[vc_audio, vc_meta, vc_xml, vc_status, active_job],
                )
                vc_stop.click(
                    fn=cancel_job,
                    inputs=[active_job],
