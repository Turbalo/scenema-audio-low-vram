...
from .whisper_aligner import unload_whisper
...
class AudioProcessor:
    ...
    async def process(self, job: ProcessJob) -> ProcessResult:
        ...
        try:
            self._progress(2, "Parsing request", "Validating prompt and options")
            if self.engine is None:
                self._progress(5, "Loading models", "Starting audio engine")
                self.startup()
            ...
            self._progress(96, "Encoding WAV", "Preparing audio response")
            wav_bytes = self._encode_wav(wav_np, sr)
            processing_ms = int((time.time() - start_time) * 1000)
            self._progress(100, "Complete", "Audio is ready")
            ...
            if self.engine is not None and not getattr(self, "_keep_resident", False):
                try:
                    self.engine.idle_offload()
                except Exception:
                    logger.debug("AudioEngine idle offload failed", exc_info=True)
            if not getattr(self, "_keep_resident", False):
                try:
                    unload_whisper()
                except Exception:
                    logger.debug("Whisper unload failed", exc_info=True)
                try:
    ...
    async def _voice_design(self, config: dict) -> tuple[np.ndarray, int]:
        ...
        self._progress(10, "Compiling prompt", "Preparing voice design prompt")
        compiled = compile_prompt(config["prompt"])
        self._progress(20, "Encoding text", "Gemma CPU streaming")
        vc, ac = self.engine.encode_text(compiled.prompt)
        self._progress(45, "Generating audio", "Running audio diffusion")
        ...
        if not config["background_sfx"]:
            self._progress(78, "Cleaning vocals", "Removing background SFX")
            wav = self._strip_background(wav, sr)

        self._progress(88, "Post-processing", "Trimming silence and normalizing")
    ...
    async def _generate(self, config: dict) -> tuple[np.ndarray, int]:
        ...
        self._progress(8, "Planning chunks", "Estimating speech duration")
        chunks = plan_chunks(
        ...
        if config["reference_voice_url"]:
            self._progress(12, "Loading reference", "Preparing uploaded voice sample")
        ...
        if config["skip_vc"] and ref_wav_path:
            self._progress(18, "Encoding reference", "Creating reference latent")
        ...
        if not config["background_sfx"]:
            self._progress(76, "Cleaning vocals", "Removing background SFX")
            wav = self._strip_background(wav, sr)

        # Cap silence — scale with pace
        self._progress(84, "Post-processing", "Trimming silence and normalizing")
        ...
        if url.startswith("file://"):
            ...
            if not os.path.isfile(local_path):
                raise FileNotFoundError(
                    "Reference audio expired or is missing. Re-upload the file."
                )
