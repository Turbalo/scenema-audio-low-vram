...
class VRAMManager:
    ...
    def finalize(self) -> None:
        ...
        forced_offload = os.environ.get("GEMMA_OFFLOAD", "").lower() in {
            "1",
            "cpu",
            "unload",
        }
        self.needs_offload = forced_offload or (
            total_model_gb + gemma_overhead_gb
        ) > self.vram_gb
        logger.info(
...
class AudioEngine:
    ...
    def __init__(
        ...
        self._vae_sr = None
        self._loaded = False
        nf4_offload_mode = os.environ.get("GEMMA_OFFLOAD", "").lower()
        self._nf4_unload = nf4_offload_mode == "unload"
        self._nf4_offload = nf4_offload_mode in {"1", "cpu", "unload"}
        self._nf4_loaded_on_gpu = False
        self._nf4_batch_encode = False
    ...
    def _ensure_nf4_gemma_gpu(self) -> None:
        if not self._nf4_offload or self._nf4_loaded_on_gpu:
            return
        logger.info("Loading NF4 Gemma to GPU")
        if self._nf4_gemma_model is None:
            self._build_nf4_gemma()
        else:
            self._nf4_gemma_model.cuda()
            self._nf4_loaded_on_gpu = True
        if hasattr(self, "_cached_emb_proc") and self._cached_emb_proc is not None:
            self._cached_emb_proc.cuda()
        torch.cuda.empty_cache()

    def _offload_nf4_gemma(self) -> None:
        if not self._nf4_offload or not self._nf4_loaded_on_gpu:
            return
        if self._nf4_unload:
            logger.info("Unloading NF4 Gemma from memory")
            del self._nf4_gemma_model
            self._nf4_gemma_model = None
            if hasattr(self, "_cached_emb_proc") and self._cached_emb_proc is not None:
                self._cached_emb_proc.cpu()
            self._nf4_loaded_on_gpu = False
            gc.collect()
            torch.cuda.empty_cache()
            return
        logger.info("Offloading NF4 Gemma to CPU")
        self._nf4_gemma_model.cpu()
        if hasattr(self, "_cached_emb_proc") and self._cached_emb_proc is not None:
            self._cached_emb_proc.cpu()
        self._nf4_loaded_on_gpu = False
        torch.cuda.empty_cache()

    def begin_text_encode_batch(self) -> None:
        """Keep NF4 Gemma resident while encoding a batch of prompts."""
        self._nf4_batch_encode = True

    def end_text_encode_batch(self) -> None:
        """Release NF4 Gemma after a batched text-encoding phase."""
        self._nf4_batch_encode = False
        self._offload_nf4_gemma()
    ...
    def idle_offload(self) -> None:
        """Move phase models off GPU after a request finishes."""
        self._offload_nf4_gemma()
        if self._vram:
            self._vram.free_all()
        if hasattr(self, "_cached_emb_proc") and self._cached_emb_proc is not None:
        ...
        gc.collect()
        torch.cuda.empty_cache()
        logger.info("AudioEngine idle offload complete")
    ...
    def _generate_impl(self, vc, ac, duration, seed, ref_latent=None):
        ...
        vc = vc.to(device="cuda", dtype=torch.bfloat16)
        ac = ac.to(device="cuda", dtype=torch.bfloat16)
